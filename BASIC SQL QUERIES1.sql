...
-- write a query to fetch trns_id, trns date, amount for net banking trns only AND FETCH ONLY SUCCESSFUL TRANS --

SELECT trns_id, trns_date, amount
FROM TRANSACTIONS
WHERE TRNS_TYPE = 'net banking' AND STATUS = 'SUCCESS';
...
-- -- 8) sort the employee data as per first name and highest SALARY. --
SELECT * FROM EMPLOYEES;
...
SELECT * FROM CUSTOMERS;
SELECT CUSTOMER_ID, CONCAT(FIRST_NAME, ' ', LAST_NAME) AS CUSTOMER_NAME, IS_ACTIVE
FROM CUSTOMERS
WHERE IS_ACTIVE = 'TRUE';

-- 4) Fetch customer id and name of all active customers who were born after 2000.
SELECT CUSTOMER_ID, CONCAT(FIRST_NAME, ' ', LAST_NAME) AS CUSTOMER_NAME, DOB
FROM CUSTOMERS
...
SELECT * FROM CUSTOMERS;
SELECT CONCAT(FIRST_NAME, ' ', LAST_NAME) AS CUSTOMER_NAME, ADDRESS, PHONE_NO
from CUSTOMERS
...
SELECT *
FROM CUSTOMERS
WHERE FIRST_NAME OR LAST_NAME LIKE '%oo%';

-- 8) Identify the total no of wire transfer transactions.--
select * from transactions;
select count(trns_type)
from transactions
...
SELECT *
FROM CUSTOMERS
WHERE ADDRESS = '77 LIEN PARK'
OR ADDRESS =  '337 WESTEND PARK'
OR ADDRESS = '9 Troy Plaza';

-- 13) Fetch all customers who have "Park" or "Plaza" in their address.
SELECT *
FROM CUSTOMERS
WHERE ADDRESS LIKE '%PARK%'
OR ADDRESS LIKE '%PLAZA%'
